...
site116 = combined_1day %>% filter(SiteId==116)
...
for (site in unique(combined_1day$SiteId)){
  ...
  site_result = predict_for_site_arima(combined_1day %>% filter(SiteId==site))
  final_1day = rbind(final_1day, site_result)
}


########### Fit auto.arima for 1 hour
# bind train with test
temp <- train_1hour %>% 
  select(SiteId, Timestamp, Value, obs_id)
temp2 <- test_1hour %>% 
...
for (site in unique(combined_1hour$SiteId)){
  print(paste("SiteId:",site))
  site_result = predict_for_site_arima(combined_1hour %>% filter(SiteId==site))
  final_1hour = rbind(final_1hour, site_result)
}


########### Fit auto.arima for 15 min
# bind train with test
temp <- train_15min %>% 
  select(SiteId, Timestamp, Value, obs_id)
temp2 <- test_15min %>% 
...
for (site in unique(combined_15min$SiteId)){
  print(paste("SiteId:",site))
  site_result = predict_for_site_arima(combined_15min %>% filter(SiteId==site))
...
##### Fit Arima
library(forecast)
temp_try = combined_1hour %>% filter(SiteId == 2) #case where NA's occur only after past obs
temp_train = temp_try[!is.na(temp_try$Value),]
fit1 = auto.arima(temp_train$Value)
summary(fit1) #p =3, d=1, q=2 (3rd order autoregress, 1 degree of difference between responses, 2nd order of MA )
predict(fit1, 190)$pred
...
for (site in unique(combined_1hour$SiteId)){
  print(paste("SiteId:",site))
  site_result = predict_for_site(combined_1hour %>% filter(SiteId==site))
  final = rbind(final, site_result)
}

##### Generate submission file
test_1hour_arima = final %>% filter(obs_id %in% test_1hour$obs_id) 
test_1hour_arima = merge(test_1hour_arima, test_1hour %>% select(obs_id, ForecastId),by='obs_id',all.x= TRUE)
test_1hour_arima = test_1hour_arima %>% select(colnames(test_1hour)) %>% arrange(SiteId, ForecastId, Timestamp)
write.csv(test_1hour_arima, file='submission/test_1hour_arima.csv', row.names = FALSE)
